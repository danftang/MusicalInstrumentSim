class MetalSheet {
    ...
    val T = 10.0           // force per unit displacement (N)
    ...
    inline fun friction(i: Int, j: Int) : Double {
        return if(i<1 || j<1 || i>X-2 || j>Y-2) fBoundary else fAir
    }


    fun d2_di2(a: DoubleArray2D , i : Int, j : Int) : Double {
        if(i == 0) return (a[i+1,j] - 2.0*a[i,j])
        if(i == a.xSize-1) return (a[i-1,j] - 2.0*a[i,j])
        return (a[i+1,j] - 2.0*a[i,j] + a[i-1,j])
    }

    fun d2_dj2(a: DoubleArray2D , i : Int, j : Int) : Double {
        if(j == 0) return (a[i,j+1] - 2.0*a[i,j])
        if(j == a.ySize-1) return (a[i,j-1] - 2.0*a[i,j])
